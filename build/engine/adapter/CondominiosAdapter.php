...
use engine\dao\Condominios;
use engine\utils\FilterWhere;

class CondominiosAdapter {
    ...
    public function getAll($where, $orderColun, $order, $page, $sizePage){
    	...
    	foreach ($listCondominios as $result){
            ...
            //AD_HOSTNAME
            $condominios->setAd_hostname($result['ad_hostname']);

            //AD_LAST_SEEN
            $condominios->setAd_last_seen($result['ad_last_seen']);

            //AD_STATUS
            $condominios->setAd_status($result['ad_status']);
